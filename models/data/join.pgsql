...
SELECT * FROM department JOIN courses ON courses.department_id = department.department_id
JOIN course_instance ON course_instance.course_instance_id = courses.course_id;
...
JOIN courses ON courses.course_id = course_instance.course_instance_id
...
-- need to add where clasue to take in param for what class
   ...
   SELECT * FROM students JOIN student_courses ON student_courses.student_id = students.student_id JOIN course_instance ON course_instance.course_instance_id = student_courses.course_instance_id JOIN courses ON courses.course_id = course_instance.course_id JOIN department ON department.department_id = courses.department_id
...
-- get all assignments from a student
   ...
   SELECT * FROM students JOIN student_courses ON student_courses.student_id = students.student_id 
   JOIN course_instance ON course_instance.course_instance_id = student_courses.course_instance_id JOIN courses ON courses.course_id = course_instance.course_id 
...
INSERT INTO course_instance(
	...
	VALUES ( '1', '1', '14', 'test-1-1');

UPDATE course_instance SET course_instance_period = 1, teacher_id = 2, course_instance_name = 'testMath-P-1' WHERE course_instance_id = 1 RETURNING *;



   SELECT * FROM department JOIN courses ON courses.department_id = department.department_id JOIN course_instance ON course_instance.course_id = courses.course_id;
